...
public class CreateReport implements CreateReportADT {
  ...
  public void monthlyFarmReport(int month, int year, boolean display) {
    ...
    if (display) {
      ...
      form.addRow(1, new Label(" Farm ID    "), new Label(" Farm Total    "),
  ...
  public void farmReport(String id, int year, boolean display) {
    ...
    if (display) {
      ...
      form.addRow(1, new Label(" Month    "), new Label(" Farm Total    "),
          new Label(" Percent Total    "));
      for (int i = 0; i < 12; i++) {
        if (singleFarm[i] == 0) {
          form.addRow((i + 1), new Label(String.valueOf(i + 1)),
              new Label(Long.toString(allFarmTotal[i])), new Label(Long.toString(0)));
        } else {
          form.addRow((i + 1), new Label(String.valueOf(i + 1)),
              new Label(Long.toString(allFarmTotal[i])),
      ...
      try {
        ...
        for (int i = 0; i < 12; i++) {
          f.write((i + 1) + "\t\t\t" + allFarmTotal[i] + "\t\t\t");
          if (singleFarm[i] == 0) {
            f.write("0\n");
          } else {
            f.write(100 * singleFarm[i] / allFarmTotal[i] + "\n");
          }
        }
      } catch (IOException e) {
        Alert alert = new Alert(AlertType.WARNING, "Unable to save data");
